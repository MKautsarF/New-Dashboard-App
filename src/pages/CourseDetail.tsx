...
const CourseDetail = () => {
  ...
  const [pageExclusion, setPageExclusion] = useState<{
  ...
  useEffect(() => {
    if (isEditing && payload) {
      const newInitialValues = {
        ...
        rainStatus:
          payload.weather?.find((item: any) => item.name === "rain")?.value ||
          "",
        fog:
          payload.weather?.find((item: any) => item.name === "fog")?.value || 0,
        time: payload.time
          ? dayjs(new Date(1970, 0, 1, ...payload.time.split(":").map(Number)))
          : null,
        motionBase: payload.motion_base || false,
        speedBuzzer: payload.speed_buzzer || false,
        speedLimit: payload.speed_limit || "",
        jarakPandang:
          payload.weather[1]?.value >= 0.5
            ? Math.round(
                Math.pow(payload.weather[1]?.value / 100, -0.914) * 50.6
              )
            : 0,
      };
  ...
  useEffect(() => {
    const hasChanges =
      ...
      (time &&
        initialValues.time &&
        !time.isSame(initialValues.time, "minute")) ||
  ...
  const handleCreateScoring = () => {
    ...
    if (useDefault) {
      navigate(
        `/Scoring?type=default&courseID=${courseId}&train=${payload.train_type}&mode=new`
      );
    } else {
      navigate(
        `/Scoring?type=${selectedScoring}&courseID=${courseId}&train=${payload.train_type}&mode=new`
      );
    }
  };
  ...
  return (
    <Container w={1000} h={700}>
          ...
          <div className="w-2/5">
            ...
            <div className="flex flex-row gap-10">
              <div className="flex flex-col gap-5">
                ...
                <InfoRow
                  label="Line Kereta"
                  value={payload.train_line}
                  isEllipsisEnabled={isEllipsisEnabled}
                  width="198px"
                />
                <InfoRow
                  label="Stasiun Mulai"
                  value={payload.route?.start?.name}
                  isEllipsisEnabled={isEllipsisEnabled}
                  width="198px"
                />
                <InfoRow
                  label="Stasiun Akhir"
                  value={payload.route?.finish?.name}
                  isEllipsisEnabled={isEllipsisEnabled}
                  width="198px"
                />
              </div>
              <div className="flex flex-col gap-5">
                <InfoRow
                  label="Status Hujan"
                  value={payload.weather?.[0]?.value}
                  isEllipsisEnabled={isEllipsisEnabled}
                  width="96px"
                />
                <InfoRow
                  label="Waktu"
                  value={`${payload.time}:00`}
                  isEllipsisEnabled={isEllipsisEnabled}
                  width="96px"
                />
                <InfoRow
                  label="Jarak Pandang"
                  value={getWeatherDisplayValue(payload.weather?.[1]?.value)}
                  isEllipsisEnabled={isEllipsisEnabled}
                  width="96px"
                />
                <InfoRow
                  label="Motion Base"
                  value={payload.motion_base ? "On" : "Off"}
                  isEllipsisEnabled={isEllipsisEnabled}
                  width="96px"
                />
                <InfoRow
                        ...
                        >
                          ...
                          <TableCell>
                            <div className="flex gap-4 justify-end">
                              <Tooltip
                                title="Detail dan Edit Modul Penilaian"
                                placement="top"
                              >
                                <IconButton
                                  size="small"
                                  onClick={() => {
                                    ...
                                    navigate(
                                      `/Scoring?type=${row.id}&courseID=${courseId}&train=${payload.train_type}&mode=edit`
                                    );
                                  }}
                                >
                                  <Info />
      ...
      <Dialog open={open} onClose={() => setOpen(false)}>
        <DialogTitle className="px-6 pt-6">Tambah Modul Penilaian</DialogTitle>
        <DialogContent className="w-[600px] px-6">
          ...
          {!useDefault && scoring?.total > 0 && (
            <FormControl fullWidth variant="standard" margin="normal">
              <DialogContentText>
                Pilih Template Modul Penilaian{" "}
              </DialogContentText>
              <Select
